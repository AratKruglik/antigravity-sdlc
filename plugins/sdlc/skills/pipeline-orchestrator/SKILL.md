...
- Installed plugins under `~/.gemini/config/plugins/**`.
...
The preflight result is cached in `~/.gemini/config/.sdlc-deps-preflight.json` to avoid repeating 11+ tool calls on every `/sdlc:start` invocation.

**Fast-path (cache hit):**

1. If `$ARGUMENTS` contains `--force-preflight`, skip to full scan below.
2. Read `~/.gemini/config/.sdlc-deps-preflight.json` (1 tool call).
...
1. Use `Glob ~/.gemini/config/plugins/**/runtime-dependencies.json` to find all declarations.
...
Write `~/.gemini/config/.sdlc-deps-preflight.json`:
...
For each entry in runtime-dependencies.json#dependencies:
  For each skill_name in entry.skills_used:
    skill_path = ~/.gemini/config/plugins/{entry.name}/skills/{skill_name}/SKILL.md
...
```
~/.gemini/config/plugins/**/stack.md
```
...
```
🔧 Local overrides applied from local configuration:
...
If a local override file exists but parsing fails (invalid YAML, unknown top-level keys), print a warning and continue with the unmodified plugin profile:

```
⚠️ Failed to parse local override file: <error>. Continuing with plugin defaults.
```
...
1. **Locate:** resolve `WORKFLOW_NAME` (from `--workflow=NAME`, or `"default"`).
   Find `~/.gemini/config/plugins/sdlc/workflows/{WORKFLOW_NAME}.yaml` via `Glob`.
...
- Modify files inside `~/.gemini/config/plugins/**`.
...
| `mcp__skills__list_skills` unavailable | Use FS fallback: check `~/.gemini/config/plugins/{plugin}/skills/{skill}/SKILL.md` exists. |
